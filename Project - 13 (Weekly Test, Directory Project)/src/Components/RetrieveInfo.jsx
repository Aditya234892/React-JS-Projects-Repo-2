    ...
    <div className="w-full min-h-[74.5vh] bg-indigo-200 p-4">
      <div className="w-full flex flex-col items-center border-4 border-red-800 bg-red-100 px-5 pb-6">
        ...
        <div className="w-full mt-10 bg-indigo-300 p-4 border-4 border-indigo-500 rounded-2xl flex flex-col items-center text-xl gap-y-3">
          ...
          >
            <option value="S"  disabled = {dropDown !== ""}>Select to Search</option>
          ...
          {dropDown === "name" && (
            <>
              ...
              <div className="px-5 py-2 mt-4 flex gap-x-5">
                {peopleData
                  .filter((person) => person.name === name) // Filter people based on name
                  .map((person, index) => (
                    <div
                      key={index}
          ...
          {dropDown === "aadhar" && (
            <>
              <h1 className="text-3xl font-semibold mt-6">
                Enter Aadhar Card Number, to Search
              </h1>
              <input
                type="number"
                placeholder="Search"
              ...
              <div className="px-5 py-2 mt-4">
                {peopleData
                  .filter((person) => person.aadharNumber === aadharNumber) // Filter people based on aadharNumber
                  .map((person, index) => (
                    <div
                      key={index}
                      className="bg-white p-4 border-2 border-indigo-400 rounded-xl mt-4"
          ...
          {dropDown === "mobile" && (
            <>
              ...
              <div className="px-5 py-2 mt-4">
                {peopleData
                  .filter((person) => person.phnNumber === mobile) // Filter people based on name
                  .map((person, index) => (
                    <div
                      key={index}
                      className="bg-white p-4 border-2 border-indigo-400 rounded-xl mt-4"
